...
class Button():
    ...
    def cook(self, data, objects, food, app, screen):
        if objects[f"{app}1"].available == data["required"][food]:
            clone = objects[f"{app}1"].new_image(objects[f"{app}1"].rect.topleft, data[f"{app}"]["size"]["width"], data[f"{app}"]["size"]["height"], data[food]["image"], screen)
            ...
            clone = objects[f"{app}2"].new_image(objects[f"{app}2"].rect.topleft, data[f"{app}"]["size"]["width"], data[f"{app}"]["size"]["height"], data[food]["image"], screen)
            objects[f"{app}2"].available = f"{food}"
            data["clone_image"][f"{app}2food"] = clone
        elif objects[f"{app}3"].available == data["required"][food]:
            clone = objects[f"{app}3"].new_image(objects[f"{app}3"].rect.topleft, data[f"{app}"]["size"]["width"], data[f"{app}"]["size"]["height"], data[food]["image"], screen)
            objects[f"{app}3"].available = f"{food}"
            data["clone_image"][f"{app}3food"] = clone
        elif objects[f"{app}4"].available == data["required"][food]:
            clone = objects[f"{app}4"].new_image(objects[f"{app}4"].rect.topleft, data[f"{app}"]["size"]["width"], data[f"{app}"]["size"]["height"], data[food]["image"], screen)
            objects[f"{app}4"].available = f"{food}"
            data["clone_image"][f"{app}4food"] = clone
